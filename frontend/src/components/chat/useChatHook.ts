...
function deriveTitleFromMessages(messages: ChatMessage[], fallback: string): string {
  ...
  const candidate = stripHtmlTags(userContent || '')
  if (!candidate) {
    return fallback
  }
  // Keep full first message (max 80 chars)
...
function useChatHook(): ChatContextValue {
  ...
  const applyState = useCallback(
    (nextList: Chat[], requestedCurrentId?: string, options?: SyncOptions) => {
      ...
      if (shouldPersist) {
        // Never persist empty chats to localStorage
        const chatsToStore = normalized.filter(
          (chat) => (messagesMapRef.current.get(chat.id) ?? []).length > 0
        )
        cacheSetJson(STORAGE_KEYS.chatList, chatsToStore)
    ...
    (messages: ChatMessage[], chatId?: string, options?: { chat?: Chat }) => {
      ...
      if (existingChatIndex !== -1) {
        nextList = baseList.map((item) => {
          ...
          const shouldDeriveTitle = isFirstMessage
          const fallbackTitle =
  ...
  const activateChat = useCallback(
    (chat: Chat, options?: { persistOutgoing?: boolean }) => {
      ...
      // If the outgoing chat is empty, remove it from the list instead of keeping it
      const baseList =
        prevId && prevId !== chat.id && outgoingMessages.length === 0 && !isStreaming
          ? chatListRef.current.filter((item) => item.id !== prevId)
          : chatListRef.current
  ...
  const onDeleteChat = useCallback(
    (chat: Chat) => {
      ...
      const nextChatId = isCurrentChatDeleted ? filteredList[0]?.id : currentChatIdRef.current

      applyState(filteredList, nextChatId)
    },
