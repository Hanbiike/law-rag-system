...
/** Strip the embedded Law RAG meta comment from assistant message text */
function cleanAssistantText(text: string): string {
  return extractMetaFromText(text).cleanText
}
...
function ThinkingDots(): React.JSX.Element {
  return (
    <span className="inline-flex items-center gap-1" aria-label="Генерирую ответ">
      {THINKING_DOTS.map((i) => (
        <span
          key={i}
          className="bg-muted-foreground/60 inline-block size-1.5 rounded-full animate-[pulse-dot_1.4s_ease-in-out_infinite] motion-reduce:animate-none"
...
function AssistantMessage({ message, isThinking }: MessageProps): React.JSX.Element {
  ...
  const markdownSource = useMemo(() => {
    const rawText = cleanAssistantText(getTextContent(deferredParts))
    return sources.length > 0 ? stripTrailingSourceMarkdownLinks(rawText, sources) : rawText
  }, [deferredParts, sources])

  const copyText = useMemo(() => cleanAssistantText(getTextContent(parts)), [parts])
