...
import { buildUserMessageParts, getTextFromParts } from '@/components/chat/chat-attachments'
import {
  ChatComposer,
...
import { extractMetaFromText } from '@/services/law-rag'
...
function Chat(_: ChatProps, ref: React.ForwardedRef<ChatRef>): React.JSX.Element {
  ...
  const { lawRagMode, lawRagLang } = useAppContext()
  const t = useUIStrings(lawRagLang)

  const [composerError, setComposerError] = useState<string | null>(null)
  const [hasActiveChat, setHasActiveChat] = useState(Boolean(currentChatId))

  // ── Law RAG response_id helpers ─────────────────────────────────────────
  const getLawRagResponseId = useCallback((chatId: string): string | null => {
    const store = cacheGetJson<Record<string, string>>(CacheKey.LawRagResponseIds, {})
    return store[chatId] ?? null
  }, [])

  const saveLawRagResponseId = useCallback((chatId: string, rid: string | null): void => {
    const store = cacheGetJson<Record<string, string>>(CacheKey.LawRagResponseIds, {})
    if (rid) {
      store[chatId] = rid
    } else {
    ...
    cacheSetJson(CacheKey.LawRagResponseIds, store)
  }, [])
  ...
  const { messages, setMessages, sendMessage, status, error, stop } = useChat<ChatMessage>({
    ...
    onFinish: ({ messages: finalMessages, isAbort }) => {
      ...
      // Extract Law RAG response_id from last assistant message
      const lastAssistant = [...normalized].reverse().find((m) => m.role === 'assistant')
      if (lastAssistant) {
        const text = getTextFromParts(lastAssistant.parts ?? [])
        const { rid } = extractMetaFromText(text)
        saveLawRagResponseId(targetChatId, rid)
      }
  ...
  const handleSend = useCallback(
    async ({ text, uploadedImages, uploadedDocuments }: ChatComposerPayload) => {
      ...
      // Read Law RAG previous_response_id for this chat
      const lrPreviousResponseId = getLawRagResponseId(activeChat.id)
  ...
  const handleClearMessages = useCallback(() => {
    ...
    if (chatId) {
      saveMessages([], chatId)
      // Reset Law RAG conversation history
      saveLawRagResponseId(chatId, null)
    }
  }, [currentChatId, isCurrentChatLoading, saveLawRagResponseId, saveMessages, setMessages, stop])
  ...
  if (messages.length === 0 && isChatHydrated) {
    return (
      <div className="bg-background text-foreground flex min-h-0 flex-1 flex-col items-center justify-center px-4">
        <div className="relative w-full max-w-2xl -translate-y-4 space-y-8 text-center">
          <div className="space-y-4">
            <h1 className="text-foreground font-display text-4xl font-medium tracking-tight text-balance md:text-5xl lg:text-6xl">
              {t.welcomeTitle}
            </h1>
            <p className="text-muted-foreground mx-auto max-w-md font-serif text-lg leading-relaxed text-pretty md:text-xl">
              {t.welcomeSubtitle}
            </p>
          </div>
          <ChatComposer
