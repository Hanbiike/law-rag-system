...
function ChatTitleSpan({ title }: { title: string }): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const wasDefault =
      prevTitleRef.current === NEW_CHAT_TITLE || prevTitleRef.current === ''
    ...
    const step = () => {
      ...
      if (i < title.length) {
        timerRef.current = setTimeout(step, TYPING_INTERVAL_MS)
      }
    }
    timerRef.current = setTimeout(step, TYPING_INTERVAL_MS)
