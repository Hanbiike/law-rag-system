...
Запрос пользователя
       ↓
[Режим 'pro'] → LLM генерирует уточняющие вопросы
[Режим 'base'/'search'] → прямая векторизация
       ↓
QueryEmbedder → векторизация запросов
       ↓
Milvus → поиск похожих статей (COSINE)
       ↓
Дедупликация результатов
       ↓
[Режим 'search'] → возврат форматированных статей
[Режим 'base'/'pro'] → LLM генерация ответа с контекстом
       ↓
Ответ пользователю
```
...
**Документы/изображения:**
```
Документ/изображение (URL)
       ↓
LLM извлекает параграфы (get_doc_data/get_image_data)
       ↓
[Режим 'base'] → векторизация каждого параграфа
               → поиск по top_k для каждого
               → контекст: paragraphs × top_k
       ↓
[Режим 'pro'] → для КАЖДОГО параграфа:
              → генерация n вопросов
              → поиск по top_k для каждого вопроса
              → контекст: paragraphs × n_questions × top_k
       ↓
Дедупликация результатов
       ↓
