...
- **📊 Два режима поиска**: базовый (1 запрос) и продвинутый (2 запроса) с генерацией уточняющих вопросов
- **🌐 Поддержка двух языков**: русский и кыргызский
- **📄 Анализ PDF документов** с извлечением структурированных данных
...
2. Выбрать режим ответа (Базовый / Продвинутый)
...
| `llm.py` | Azure OpenAI клиент с новым responses API (`responses.parse`, `responses.create`) |
...
| `keyboards.py` | Кэшированные inline/reply клавиатуры |
...
- Валидация размера документов (макс. 20 МБ)
